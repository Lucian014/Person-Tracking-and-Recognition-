...
TRACKER_CONFIG_FILE = os.path.join(SCRIPT_DIR, "memory.yaml")
REID_SIMILARITY_THRESH = 0.69  # cât de similare trebuie să fie feature-urile pentru match
...
class PersonTracking:
    ...
    def find_match(self, features, bbox, current_tracker_id):
        ...
        if candidates:
            best_pid, best_score, best_sim, best_spatial = candidates[0]
            self.all_iou_scores.append(best_spatial) # Colectăm IoU
            self.all_sim_scores.append(best_sim)     # Colectăm Similaritatea
    ...
    def update(self, frame, detections):
        ...
        for track_id, bbox in detections_sorted:
            ...
            if track_id in self.tracker_to_permanent:
                ...
                if matched_id:
                    permanent_id = matched_id
                    self.all_reid_scores.append(score)
                else:
...
while True:
    ...
    cv2.imshow("Proiect PI", final_frame)

    key = cv2.waitKey(1) & 0xFF
    if key == ord('q') or key == ord('Q') or cv2.getWindowProperty("Proiect PI", cv2.WND_PROP_VISIBLE) < 1:
        break

cap.release()
cv2.destroyAllWindows()
print("\n" + "="*30)
print("   RAPORT EVALUARE TRACKING")
print("="*30)


# 2. Calculăm IoU mediu (consistența spațială)
iou_mediu = np.mean(reid.all_iou_scores) if reid.all_iou_scores else 0

# 3. Calculăm similaritatea medie 
medie_sim_intre_frames = np.mean(reid.all_sim_scores) if reid.all_sim_scores else 0

medie_de_reidentificare = 0
if reid.all_reid_scores:
    medie_de_reidentificare = sum(reid.all_reid_scores) / len(reid.all_reid_scores)

stats = reid.get_stats()

print(f"[*] Persoane unice detectate: {stats['total']}")
print(f"[*] Similaritate între cadre: {medie_sim_intre_frames:.2f}")
